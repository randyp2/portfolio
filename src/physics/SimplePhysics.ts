...
export class SimplePhysics {
  ...
  setColliders(colliders: ColliderRect[]) {
    colliders.forEach((c) => {
      console.log(
        `Collider: leftX:${c.leftX}, rightX:${c.rightX}, topY:${c.topY}, bottomY:${c.bottomY}`,
      );
    });
    this.colliders = colliders;
  }
  ...
  private resolveBlockBlockCollision(
    ...
    // Vertical collision (no bouncing, just stack)
    // Separate blocks vertically so they stack
    const separation = overlapY / 2 + 0.5; // Small buffer to prevent overlap
    ...
    // Stop vertical movement for both blocks
    block1.vy = 0;
    block2.vy = 0;
